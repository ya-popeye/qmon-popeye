...
import org.mockito.stubbing.{OngoingStubbing, Stubber, Answer}
...
trait MockitoStubs extends MockitoSugar {
  /** delegate to MockitoMocker doAnswer with a MockAnswer object using the function f. */

  def doAnswer[T](f: Any => T) = org.mockito.Mockito.doAnswer(new MockAnswer(f))
  ...
  class Stubbed[T](c: => T) {
    def returns(t: T, t2: T*): OngoingStubbing[T] = {
      if (t2.isEmpty)
        org.mockito.Mockito.when(c).thenReturn(t)
      else
        t2.foldLeft(org.mockito.Mockito.when(c).thenReturn(t)) { (res, cur) => res.thenReturn(cur)}
    }

    def answers[P](function: Any => T) = org.mockito.Mockito.when(c).thenAnswer(new MockAnswer(function))
    ...
    def throws[E <: Throwable](e: E*): OngoingStubbing[T] = {
      ...
      e.drop(1).foldLeft(org.mockito.Mockito.when(c).thenThrow(e.head)) { (res, cur) => res.thenThrow(cur)}
    }
  }
